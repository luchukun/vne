...
void KSP_my(int d, int s)
{
	...
	int i,i_s,j_s,j,m,n,spurNode,rootnode,flag;
	...
	for(i=i_s;i<n_server-1;i++)
		D[i] = D[i+1];
	for(i=0;i<n_switch;i++)
		D[n_server+i-1] = switch_set[i];
	n_D = n_node-1;
	...
	while( k < K-1 )
	{
		for(i=1;i<pathSet[k][0];i++) //对path中的每条边依次删去
		{
			...
			//---初始化D[]: 
			for(j=0;j<n_server;j++)
			{
				D[j] = server_set[j];
				if(D[j]==s)
					j_s = j;
			}
			for(j=j_s;j<n_server-1;j++)
				D[j] = D[j+1];
			for(j=0;j<n_switch;j++)
				D[n_server+j-1] = switch_set[j];
			n_D = n_node-1;
...
void figure()
{
	...
	fout.open("E:\\data\\KSP\\BCube\\bn-B\\1.txt");
	fout.clear();
	fout.close();
	/*fout.open("E:\\data\\KSP\\BCube\\bw-B\\1.txt");
	fout.clear();
	fout.close();
	fout.open("E:\\data\\KSP\\BCube\\suc-B\\1.txt");
	fout.clear();
	fout.close();*/

	fout.open("E:\\data\\KSP\\BCube\\bn-B\\1.txt",ios::app);
	for(i=1;i<=cycle;i++)
		fout<<bn_array[i]<<" ";
	fout.close();

	/*fout.open("E:\\data\\KSP\\BCube\\bw-B\\1.txt",ios::app);
	for(i=1;i<=cycle;i++) 
		fout<<bw_array[i]<<" ";
	fout.close();

	fout.open("E:\\data\\KSP\\BCube\\suc-B\\1.txt",ios::app);
	for(i=1;i<=cycle;i++)
