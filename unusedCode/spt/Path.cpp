...
{
  public:
    Graph( DistType Infinity, const char *FileName = NULL );
    ~Graph( );
...
Graph<NameType, DistType>::Dijkstra( int StartNode )
{
    ...
    for( int NodesSeen = 0; NodesSeen < NumVertices; NodesSeen++  )
    {
        ...
        V = VRec.Dest;
        Table[ V ].Scratch = 1;     // Visited vertex as being seen
        for( ListItr< Edge<DistType> > P = *Table[ V ].Adj; +P; ++P )
        {
            W = P( ).Dest;
            DistType Cvw = P( ).Cost;
...
template <class NameType, class DistType>    
Graph<NameType,DistType>::Graph( DistType Infinity,
    const char *FileName ) : NumVertices( 0 ), Infinity( Infinity ),
                             Table( InitTableSize )
{
    if( FileName != NULL )
    {
