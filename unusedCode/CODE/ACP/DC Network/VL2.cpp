/* Perturbation algorithm on VL2 network */
/*
...
void Generate()
{
	...
	for(i=0;i<2;i++)
		for(j=0;j<2;j++)
			for(k=0;k<3;k++)
			{
				weigh[3+i*2+j][7+i*3+k] = 1;
				link[3+i*2+j][7+i*3+k] = rand()%50+50;//75
			}
...
void KSP_my(int d, int s)
{
	...
	int i,j,m,n,spurNode,rootnode,flag,min_index;
...
void figure()
{
	...
	fout.open("E:\\data\\KSP\\VL2\\bn-B\\8.txt");
	fout.clear();
	fout.close();
	/*fout.open("E:\\data\\KSP\\VL2\\bw-B.txt");
	fout.clear();
	fout.close();*/
	fout.open("E:\\data\\KSP\\VL2\\suc-B\\8.txt");
	fout.clear();
	fout.close();

	fout.open("E:\\data\\KSP\\VL2\\bn-B\\8.txt",ios::app);
	for(i=1;i<=cycle;i++)
		fout<<bn_array[i]<<" ";
	fout.close();

	/*fout.open("E:\\data\\KSP\\VL2\\bw-B.txt",ios::app);
	for(i=1;i<=cycle;i++) 
		fout<<bw_array[i]<<" ";
	fout.close();*/

	fout.open("E:\\data\\KSP\\VL2\\suc-B\\8.txt",ios::app);
	for(i=1;i<=cycle;i++)
